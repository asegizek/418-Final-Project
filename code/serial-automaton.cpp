...
SerialAutomaton::SerialAutomaton() {
  num_iters = 0;
  grid = NULL;
  curr_grid = NULL;
  next_grid = NULL;
...
Grid* SerialAutomaton::get_grid() {
  grid->data = curr_grid;
  return grid;
}


void SerialAutomaton::setup(int num_of_iters) {
...
void SerialAutomaton::set_rule(Rule *_rule) {
  rule = _rule;
...
void SerialAutomaton::create_grid(char *filename) {
  FILE *input = NULL;
  int width, height;
  grid_elem *data;
...
void SerialAutomaton::run_automaton() {
...
void SerialAutomaton::update_cells() {
  int grid_index;
