...
ParallelAutomaton::ParallelAutomaton() {
  num_iters = 0;
  grid = NULL;
...
void
ParallelAutomaton::setup(int num_of_iters) {
...
void
ParallelAutomaton::create_grid(char *filename) {
  FILE *input = NULL;
  int width, height;
  grid_elem *data;
...
//single update
void ParallelAutomaton::update_cells() {
...
void ParallelAutomaton::run_automaton() {
...
void ParallelAutomaton::set_rule(Rule *_rule) {
  rule = _rule;
  return;
}
